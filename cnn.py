...
def load():
    # 加载数据
    print('开始加载数据ing')
    train = pd.read_excel('data/train.xlsx', usecols = [0,1],nrows=50000,encoding='utf-8')  # 读取excel文件
    print('加载完毕')    
...
def get_dict():    
    # 载入词典
    print('开始加载词典ing')
    index_dict=np.load('other/w2n_dic.npy',allow_pickle=True).item()
    word_vectors=np.load('other/w2v_dic.npy',allow_pickle=True).item()
    print('加载完毕')
    # 加载词向量矩阵
    print('开始加载词向量ing')
